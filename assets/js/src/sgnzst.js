...
function animate() {
  function animasiWindowScroll(elemen, animasi) {
    function if_view() {
      ...
      let window_top_scroll = $(window).scrollTop();
      let window_bottom_scroll = (window_height + window_top_scroll);
      $.each(elemen, function(){
        let element_height = $(this).outerHeight();
        let element_diatas_plus = $(this).offset().top + 120;
        let element_dilihat = ((element_height + element_diatas_plus) >= window_top_scroll) && (element_diatas_plus <= window_bottom_scroll)
        if (element_dilihat) {
          setTimeout(() => {
